...
export default function StudentScan() {
  ...
  const selectedEnrollment = useMemo(
    () => {
      ...
      return (
        enrollments.find((item) => item.activeRoundId || item.activeSessionId) ??
        enrollments[0]
      );
    },
  ...
  const handleScan = useCallback(
    async (data: string): Promise<ScanResult> => {
      ...
      if (user?.must_change_password) {
        const message = "Change your temporary password before scanning attendance.";
        toast({ variant: "destructive", title: "Password update required", description: message });
  ...
  const handleChangePassword = useCallback(
    ...
    [passwordState, queryClient, toast],
  );
          ...
          <Card className="border-border/70 shadow-sm">
            ...
            <CardContent>
              <p className="text-sm text-muted-foreground">
                Restoring your student account after reload.
              </p>
            </CardContent>
          </Card>
        ) : !isAuthenticated ? (
          ...
          <>
            <AlertDialog open={mustChangePassword}>
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    Your account is using the temporary password 00000000.
                    Update it now before using attendance features.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                  ...
                  {enrollmentsQuery.data?.enrollments?.length ? (
                    <div className="space-y-2">
                      {enrollmentsQuery.data.enrollments.map((item) => (
                        ...
                        >
                          <p className="font-medium">{item.course.code} - {item.course.name}</p>
                          <p className="text-muted-foreground">
                            Group {item.group.name} - {item.group.meeting_schedule ?? item.course.term}
                          </p>
                        </div>
                      ))}
                    </div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      You are not assigned to a class yet. Ask your professor to assign you.
                    </p>
                  )}
                </CardContent>
              </Card>
            ...
            <div className="grid gap-4 lg:grid-cols-[320px_1fr]">
              ...
              <div className="space-y-4 order-1 lg:order-2">
                {selectedEnrollment && !mustChangePassword ? (
                  <Card className="border-border/70 shadow-sm">
                    <CardHeader>
                  ...
                  <Card className="border-border/70 shadow-sm">
                    ...
                    <CardContent>
                      <p className="text-sm text-muted-foreground">
                        {mustChangePassword
                          ? "Change your password to unlock the QR scanner."
                          : "You need a class assignment before scanning."}
                      </p>
                    </CardContent>
                  </Card>
                )}
