...
import { emitRoundQrUpdated, emitSessionRefresh } from "../websocket/manager";
...
async function ensureStudentCanScan(
  executor: DbExecutor,
  studentId: string,
) {
  ...
  if (student?.must_change_password) {
    throw new ApiError(
      403,
      "You must change your password before scanning attendance.",
...
export const attendanceService = {
  ...
  async recordScan(
    ...
    await ensureStudentCanScan(db, studentId);
    ...
    const roundStartMs = round.starts_at
      ...
      : nowMs;
    await policyService.getActivePolicyForRound(
      session.course_id,
    ...
    emitSessionRefresh(session.id, {
      sessionId: session.id,
      reason: "scan_recorded",
      roundId,
      studentId,
      studentName: student?.displayName ?? undefined,
      status: isLate ? "late" : "on_time",
      recordedAt,
    });
