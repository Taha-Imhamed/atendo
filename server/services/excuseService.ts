...
import { emitSessionRefresh } from "../websocket/manager";
...
export const excuseService = {
  async submitExcuse(
    ...
    emitSessionRefresh(roundInfo.session.id, {
      sessionId: roundInfo.session.id,
      reason: "excuse_submitted",
      roundId: payload.roundId,
      studentId,
      excuseId: excuse.id,
    });
  ...
  async reviewExcuse(
    ...
    emitSessionRefresh(excuseRow.session.id, {
      sessionId: excuseRow.session.id,
      reason: "excuse_reviewed",
      roundId: targetRoundId,
